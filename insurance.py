...
class LossRatioPredictor:
    def __init__(self):
        self.model = None
        self.df = pd.DataFrame()  # Initialize with empty DataFrame
    ...
    def load_sample_data(self):
        ...
        np.random.seed(42)
        n_samples = 1000  # Increased sample size for better model training

        # Generate correlated features
        age = np.random.randint(22, 65, n_samples)
        base_premium = np.random.uniform(20000, 50000, n_samples)

        # Create relationships between features
        claim_prob = np.clip(0.1 + (age - 30) * 0.005 + np.random.normal(0, 0.02, n_samples), 0, 1)
        last_year_claims = np.random.binomial(1, claim_prob, n_samples)

        # Generate realistic loss ratios with some patterns
        loss_ratios = np.where(
            last_year_claims == 1,
            np.random.uniform(0.3, 1.2, n_samples),
            np.random.uniform(0, 0.3, n_samples)
        )

        # Add more realistic patterns
        loss_ratios = np.where(
            (age > 50) & (last_year_claims == 1),
            loss_ratios * 1.3,  # Older claimants with claims have higher loss ratios
            loss_ratios
        )

        # Generate chronic conditions and family history data with None as valid option
        chronic_conditions = np.random.choice(["None", "Hypertension", "Diabetes", "Asthma", "Heart Disease"],
                                            n_samples, p=[0.6, 0.15, 0.15, 0.05, 0.05])
        family_history = np.random.choice(["None", "Heart Disease", "Cancer", "Diabetes", "Hypertension"],
        ...
        data = {
            ...
            "Previous_Claims": np.random.poisson(0.7, n_samples),
            "Policy_Type": np.random.choice(["Inpatient Only", "Inpatient+Outpatient", "Comprehensive"],
                                          n_samples, p=[0.4, 0.4, 0.2]),
            "Sum_Insured": np.random.choice([300000, 500000, 750000, 1000000], n_samples, p=[0.3, 0.4, 0.2, 0.1]),
            "Deductible": np.random.choice([0, 5000, 10000, 15000], n_samples, p=[0.2, 0.5, 0.2, 0.1]),
            "Co_Payment": np.random.choice([0, 10, 15, 20], n_samples, p=[0.3, 0.4, 0.2, 0.1]),
            "Chronic_Conditions": chronic_conditions,
            "Family_History": family_history,
            "Last_Year_Claims": last_year_claims,
            "Annual_Premium": (base_premium / 1000).round() * 1000,
    ...
    def train_model(self, df):
        ...
        if 'Loss_Ratio_Category' in df.columns:
            class_counts = df['Loss_Ratio_Category'].value_counts()
            ...
            st.plotly_chart(fig, use_container_width=True, key=f"class_dist_pie_{len(st.session_state.get('performance_history', []))}")
...
# Auto-retraining scheduler (simulated)
if st.session_state.get('last_retraining'):
    last_retrain = st.session_state['last_retraining']
    next_retrain = last_retrain + timedelta(days=7)  # Weekly by default
    if datetime.now() > next_retrain and not predictor.df.empty:
        st.sidebar.warning("Auto-retraining due!")
        if st.sidebar.button("Run Auto-Retraining Now", key="auto_retrain_btn"):
            with st.spinner("Running scheduled retraining..."):
                predictor.auto_retrain(predictor.df.sample(frac=0.3))  # Simulate new data

# Footer
st.sidebar.markdown("---")
st.sidebar.markdown("**Loss Ratio Categories:**")
st.sidebar.markdown("- Low: < 20%")
st.sidebar.markdown("- Medium: 20-50%")
st.sidebar.markdown("- High: 50-80%")
st.sidebar.markdown("- Very High: > 80%")
